...
| 최종 목표 | 아군 알 5개를 연결하고 이어지는 전투가 끝날 때까지 해당 5목 유지 | 확정 |
| 매치 승리 | 최대 3게임 중 먼저 2게임 승리 | 확정 |
| 핵심 선택 | 5칸 상점, 유닛 선택, 리롤, 배치 위치 | 확정 |
| 전투 | 양측이 알을 하나씩 두거나 한쪽이 5목을 완성하면, 보드 위 생존 유닛 전체가 제자리에서 자동 전투 | 확정 |
...
| 게임 | 빈 바둑판에서 시작해 한쪽이 다섯 목을 완성하고 전투에서 유지할 때까지의 한 판 |
| 사이클 | 플레이어와 COM이 각각 알 하나를 배치하고 전투까지 마치는 단위 |
| 배치 차례 | 한쪽이 상점을 이용하고 알 하나를 놓는 단계 |
| 전투 | 한 사이클의 두 배치가 끝나거나 어느 한쪽이 5목을 완성하면 즉시 진행되는 자동 전투 |
...
flowchart TD
    ...
    B --> C["현재 진영 배치"]
    C --> D{"5목 완성?"}
    D -- "예" --> E["도전 진영 기록 / 즉시 전투"]
    D -- "아니요" --> F{"양 진영이 각 1회 배치?"}
    F -- "아니요" --> C
    F -- "예" --> G["일반 자동 전투"]
    E --> H["사망 유닛 제거 / 생존 체력 유지"]
    G --> H
    H --> I{"도전 진영의 5목 생존?"}
    I -- "예" --> J["게임 승리 수 +1"]
    I -- "도전 없음 또는 5목 파괴" --> C
...
- 가로, 세로, 두 대각선 중 한 방향으로 같은 진영의 알 5개를 연결하고, 즉시 시작되는 전투가 끝날 때까지 하나 이상의 5목을 유지하면 승리한다.
- 이미 알이 있는 위치에는 새 알을 놓을 수 없다.
- 한 번 확정한 배치는 플레이어가 임의로 취소할 수 없다.
- 알을 놓은 직후 새로 놓은 알을 포함한 다섯 목이 완성되면 해당 진영을 5목 도전 진영으로 기록하고 즉시 전투를 시작한다.
- 5목 도전 전투가 끝나면 도전 진영의 보드 전체를 다시 검사한다. 하나 이상의 5목이 남아 있으면 승리하고, 모두 끊겼으면 게임을 계속한다.
- 한 번의 배치로 여러 5목을 동시에 만들 수 있으므로 최초로 감지한 한 줄만이 아니라 도전 진영의 모든 연결을 판정한다.
- 현재 전투에서는 유닛 이동·소환·진영 변경이 없으므로 상대 진영의 5목이 전투 중 새로 생길 수 없으며, 양쪽 5목 동시 승리나 그로 인한 무승부는 판정하지 않는다.
...
- 모든 게임에서 COM이 흑으로 선공하고 플레이어는 백으로 후공한다.
- 한 게임 안에서는 표준 오목처럼 선공과 후공이 끝까지 한 수씩 번갈아 둔다.
- 선공과 후공이 각각 한 번 배치하면 자동 전투를 시작한 뒤 같은 순서로 다음 사이클을 진행한다.
- 단, 선공이 첫 배치에서 5목을 완성하면 후공의 대응 배치 없이 즉시 전투한다. 해당 5목이 끊기면 마지막 배치자의 상대인 후공부터 배치를 이어간다.

### 7.3 MVP 예외 처리

- 빈 교차점이 하나도 없고 어느 쪽도 다섯 목을 만들지 못하면 무승부로 처리한다.
- 무승부는 매치 승리 수에 포함하지 않으며, 같은 선공 순서로 해당 게임을 다시 시작한다.
...
- 전투 종료 후 현재 배치 진영의 생존 유닛 수와 상대 생존 유닛 수를 비교한다.
- 유닛 수 열세는 최대 3기까지만 상점 등급 확률에 반영한다.
- 무료 갱신과 유료 리롤에 동일한 보정 확률을 사용한다.
- 유닛 수가 같아지거나 앞서면 기본 확률로 즉시 복구한다.
- 플레이어와 COM에 완전히 동일하게 적용한다.
...
- 유닛 수뿐 아니라 현재 체력과 등급을 포함한 보드 전력 보정

합성, 경험치, 상점 레벨까지 한 번에 도입하면 오목보다 경제 관리의 비중이 커지므로 현재 범위에서는 제외한다.
...
등급은 등장 확률과 성능 예산을 정하고, 역할군은 모든 유닛이 공유하는 전투 규칙을 정한다. 고유 능력은 역할군과 별도로 적용한다.

| 역할군 | 공통 규칙 |
| --- | --- |
| 탱커 | 공격자의 사거리 안에 있으면 다른 역할군보다 우선 공격 대상이 된다. |
| 전사 | 사거리 1의 근접 기본공격을 사용한다. |
| 원거리 딜러 | 원거리 기본공격 또는 특수 탄환을 사용한다. |
| 마법 | 기본공격 대신 메테오나 연쇄 번개 같은 공격 주문을 사용한다. |
| 보조 | 기본공격을 사용할 수 없고 회복·강화·약화·오라 행동만 수행한다. 유효한 지원 행동이 없으면 대기한다. |

### 9.3 유닛 명단과 초기 능력치

| 등급 | 이름 | 역할군 | 체력 | 위력 | 사거리 | 주기 | 고유 능력 |
| --- | --- | --- | ---: | ---: | ---: | ---: | --- |
| 일반 | 방패병 | 탱커 | 180 | 공격 8 | 1 | 1.4초 | 탱커 우선 공격 대상 |
| 일반 | 검사 | 전사 | 120 | 공격 18 | 1 | 1.0초 | 기본 근접공격 |
| 일반 | 궁수 | 원거리 딜러 | 85 | 공격 14 | 2 | 1.2초 | 기본 원거리 공격 |
| 일반 | 치유사 | 보조 | 90 | 회복 11 | 1 | 1.4초 | 범위 내 부상 아군 전체 회복 |
| 희귀 | 폭탄병 | 전사 | 90 | 공격 10 | 1 | 1.4초 | 사망 시 주변 모든 유닛에게 40 피해, 아군 포함 |
| 희귀 | 닌자 | 전사 | 105 | 공격 6/26 | 1 | 0.7초 | 주변 1칸에 아군이 없을 때 공격력 26 |
| 희귀 | 철갑 수호자 | 탱커 | 270 | 공격 7 | 1 | 1.6초 | 받는 피해 15% 감소 |
| 희귀 | 전투 사제 | 보조 | 120 | 회복 22 | 2 | 1.5초 | 체력 비율이 가장 낮은 아군 하나 회복 |
| 영웅 | 스나이퍼 | 원거리 딜러 | 90 | 공격 30 | 5 | 2.0초 | 8방향 직선 관통 공격 |
| 영웅 | 주술사 | 보조 | 120 | 지원 | 2 | 상시 | 범위 내 적 공격력 30% 감소, 중첩 없음 |
| 영웅 | 치어리더 | 보조 | 145 | 지원 | 1 | 상시 | 주변 공격형 아군 공격속도 30% 증가, 중첩 없음 |
| 영웅 | 마법사 | 마법 | 105 | 공격 38 | 3 | 3.0초 | 대상 주변 1칸 메테오 범위 피해 |
| 전설 | 고대 수호자 | 탱커 | 420 | 공격 9 | 1 | 1.8초 | 주변 비탱커 아군 피해 30% 대신 받기 |
| 전설 | 불사조 기사 | 전사 | 190 | 공격 24 | 1 | 0.9초 | 생애 최초 치명 피해 시 체력 50% 부활과 주변 적 40 피해 |
| 전설 | 폭풍 현자 | 마법 | 130 | 공격 34 | 3 | 2.4초 | 최대 3명에게 100%·70%·45% 연쇄 피해 |
| 전설 | 생명의 성녀 | 보조 | 160 | 회복 24 | 3 | 2.0초 | 광역 회복, 전투당 한 번 아군 치명 피해를 막고 30 회복 |
...
- 어느 한쪽이 5목을 완성하면 사이클의 배치 횟수와 관계없이 즉시 자동 전투를 시작한다.
- 전투는 제한 시간 동안 진행된다.
...
3. 양측이 알을 하나씩 배치하거나 어느 한쪽이 5목을 완성하면 보드 위의 모든 생존 유닛이 전투에 참여한다.
4. 체력이 0이 된 유닛은 즉시 사망하며 바둑판에서도 제거된다.
5. 생존한 유닛은 잃은 체력을 회복하지 않고 다음 사이클로 가져간다.
6. 일반 전투 후에는 다음 배치를 진행한다. 5목 완성으로 시작된 전투 후에는 도전 진영의 모든 5목을 다시 판정한다.
7. 5목이 모두 끊겼다면 마지막으로 배치한 진영의 상대부터 다음 배치를 진행한다.
...
- 목표 우선순위까지 같은 적이 여러 명이면 먼저 배치된 유닛을 선택한다.
- 제한 시간에 도달하면 아직 적용되지 않은 공격과 회복은 취소한다.
- 같은 시점에 행동 가능한 유닛은 배치 순서대로 처리한다. 앞선 행동으로 사망한 유닛은 행동하지 않는다.

### 10.6 전투 배속

- 배치와 전투 단계에서 항상 보이는 원형 버튼으로 x1, x2, x3, x4, x5 배속을 선택할 수 있다.
- 기본값은 x1이며 원형 버튼을 누를 때마다 x1 → x2 → x3 → x4 → x5 → x1 순서로 변경한다.
- 선택한 배속은 배치 단계에서는 실제 게임 시간에 적용하지 않지만 다음 전투에도 유지한다.
- 전투 종료, 게임 결과, 새 게임 시작 시 실제 시간 배율은 x1로 복구한다.

## 11. 승리와 매치 구조

### 11.1 게임 승리

자신의 배치 차례에 새 알을 놓아 같은 진영의 알 5개 이상을 연결하면 5목 도전을 시작하고 즉시 전투에 돌입한다. 전투 종료 후 도전 진영의 5목이 하나 이상 유지된 경우에만 게임에서 승리한다. 모두 끊겼다면 승리하지 못하고 마지막 배치자의 상대 차례부터 게임을 계속한다. 선공이 5목을 먼저 완성한 경우 후공의 대응 배치를 생략하는 것이 고정 후공 플레이어에 대한 선공 보정으로 작동한다. 유닛 전멸은 별도의 승리 조건이 아니다.
...
1. 즉시 다섯 목을 만들 수 있으면 5목 도전을 시작하는 수를 둔다.
2. 플레이어가 다음 수에 5목 도전을 시작할 수 있으면 막는다.
...
MVP 구현은 15×15의 모든 빈칸과 상점 유닛 조합을 1수 평가한다. 자신의 즉시 5목을 최우선, 상대의 즉시 5목 차단을 차순위로 두고, 네 방향의 모든 5칸 창과 연속 연결을 검사해 2·3·4목 및 열린 끝에 단계별 가중치를 준다. 연결에 포함된 기존 유닛의 현재 체력도 내구도에 반영한다. 유닛 평가는 체력, 초당 공격·회복량, 사거리 안의 적과 아군, 예상 피격량, 힐러 지원, 공격 대상의 연결 중요도를 사용한다. 동점 행동의 반복을 줄이는 수준의 작은 무작위 값만 마지막에 더한다.
...
- 상단: 플레이어와 COM의 게임 승리 수, 현재 선공, 전투 타이머, 원형 전투 배속 버튼
- 측면 또는 툴팁: 선택 유닛의 역할, 체력, 공격력, 사거리, 능력
- 상점 카드: 유닛 이름 옆에 역할군 아이콘을 표시하고, 등급은 텍스트·세로 띠·옅은 카드 배경색으로 구분
- 유닛 호버 상세: 고유 능력 정보와 분리된 역할군 패널에서 역할 이름과 공통 규칙을 설명
- 배치와 전투 중: x1~x5 전투 배속 버튼을 항상 표시하되 선택한 배속은 전투에만 적용
...
- 기본 휴리스틱 COM
- 최대 3게임의 2선승 매치 진행
- 5목 완성 시 즉시 전투하고 생존한 5목으로 승리 판정
- 배치와 전투 중 항상 표시되고 전투에만 적용되며 선택값을 유지하는 x1~x5 배속 버튼
...
- 후공 플레이어의 승률을 기록하고 COM 난이도와 전투 수치를 조정한다.
- 선공의 즉시 5목 전투 진입률과 전투 후 승리 전환율을 별도로 기록해 선공 보정 강도를 확인한다.
...
1. 상점 잠금 기능을 MVP 이후에 포함하는가?
2. 유닛마다 희귀도와 등장 확률을 다르게 둘 것인가?

### P2 — 표현과 세계관

3. 게임의 분위기와 세계관은 진지한 판타지, 캐주얼, 동양풍 중 어느 방향인가?
4. 흑·백 양쪽이 동일 유닛을 공유하는가, 진영별 고유 유닛이 있는가?
5. 목표 플레이 시간은 매치 기준 몇 분인가?
...
| 2026-08-07 | 배치로 오목 완성 시 즉시 승리 | 2026-08-08의 5목 생존 전투 규칙으로 대체 |
...
| 2026-08-07 | 빈 보드 칸이 없으면 무승부로 같은 선공 게임 재시작 | MVP 예외 상황의 진행 정지 방지 |
| 2026-08-07 | 제한 시간 이후 미적용 행동은 취소 | 전투 종료 시점을 명확히 유지 |
| 2026-08-07 | 같은 시점의 행동은 배치 순서대로 처리 | MVP 전투 결과를 결정적으로 재현 |
| 2026-08-07 | 동일 우선순위 대상은 먼저 배치된 유닛 선택 | 목표 선택 결과를 결정적으로 재현 |
| 2026-08-07 | COM은 흑으로 선공하고 플레이어는 백으로 후공 | 플레이어가 상대 배치를 보고 대응하는 흐름 채택 |
| 2026-08-08 | 5목 완성 시 즉시 전투하고 전투 후 생존 여부로 승리 판정 | 오목과 자동 전투를 승리 조건에서 직접 결합 |
| 2026-08-08 | 선공의 5목 즉시 전투는 후공 대응 배치 없이 시작 | 고정 후공 플레이어의 정보·대응 우위 보정 |
| 2026-08-08 | 5목이 파괴되면 마지막 배치자의 상대부터 게임 계속 | 즉시 전투 후 같은 진영의 연속 배치 방지 |
| 2026-08-08 | 항상 표시되는 x1~x5 순환 버튼의 배속을 전투에만 적용하고 선택값 유지 | 반복 전투 관전 속도를 사용자가 미리 선택하고 조절 |
